...
bool applyMigrations(sqlite3* db, int targetVersion)
{
    ...
    if (current < 4 && targetVersion >= 4) {
        ...
        if (!exec("INSERT OR IGNORE INTO settings (key, value) VALUES ('behaviorStreamEnabled', '0');")
            ...
            || !exec("INSERT OR IGNORE INTO settings (key, value) VALUES ('learningMemMbMax', '768');")
            ...
            || !exec("INSERT OR IGNORE INTO settings (key, value) VALUES ('onlineRankerMinExamples', '60');")
            || !exec("UPDATE settings SET value = '768' WHERE key = 'learningMemMbMax' AND value = '256';")
            || !exec("UPDATE settings SET value = '60' WHERE key = 'onlineRankerMinExamples' AND value = '120';")
